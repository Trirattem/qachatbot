...
class ProgressTracker {
  constructor(spreadsheetId) {
    ...
    this.filePath = path.join(
      config.paths.logs,
      `progress_${spreadsheetId.slice(0, 20)}.json`
    );
    this.data = null;
  }
  ...
  load() {
    if (fs.existsSync(this.filePath)) {
      try {
        ...
        const written   = Object.values(this.data.completed).filter(v => v.writtenToSheets).length;
        const pending   = total - written;
        logger.info(`📂 โหลด checkpoint: ${this.filePath}`);
        logger.info(`   ทำไปแล้ว ${total} case  |  เขียน Sheets แล้ว ${written}  |  pending ${pending}`);
  ...
  save(testId, index, result) {
    ...
    this.data.completed[testId]   = {
      ...
      writtenToSheets:  false,   // ← ยังไม่ได้ write
    };
    this._flush();
    logger.debug(`💾 checkpoint: ${testId} (${result.status})`);
  }

  // ทำเครื่องหมายว่า write Sheets แล้ว
  ...
  // นับ pending (ยังไม่ได้ write Sheets)
  get pendingSheetsCount() {
    return Object.values(this.data?.completed ?? {}).filter(v => !v.writtenToSheets).length;
  }
