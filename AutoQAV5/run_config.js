...
function autoProfileName(from, to, totalRows, dirCode, parCode) {
  ...
  const dirShort = findDir(dirCode).label;
  const parShort = findPar(parCode).label;
  return `TC_${from}-${toLabel} · ${dirShort} · ${parShort}`;
}
...
async function configureProfile(defaults, totalRows, targetType) {
  ...
  const datasets = listDatasets(targetType);
...
async function main() {
  ...
  while (true) {
    ...
    if (mainMenuChoice === '1' || mainMenuChoice === '2') break;
  }
  blank();
