...
Automated chatbot testing system built with **Playwright** + **Node.js** + **Google Sheets API**.
...
│   ├── modules/
│   │   ├── sheetsClient.js       ← Google Sheets read/write + colour
...
Your sheet must have **at minimum** these columns starting from row 1:

| A | B | C | D | E | F | G |
|---|---|---|---|---|---|---|
| Test Case ID | Question | Expected Answer | Actual Result | Status | Timestamp | Screenshot Path |
| TC_001 | ราคาสินค้าเท่าไหร่? | ราคาเริ่มต้นที่ 199 บาท | | | | |
| TC_002 | วิธีสมัครสมาชิก | กรอกอีเมลและรหัสผ่าน... | | | | |

Columns **D–G** will be filled automatically by this script.
...
```dotenv
# ── Required ──────────────────────────────────────────────────
# Get this from the sheet URL: /spreadsheets/d/THIS_PART/edit
...
### Dry run (reads Sheets but does NOT write results back)
```bash
npm run test:dry
```
...
### Google Sheets write permission denied
